...
load_icode(int fd, int argc, char **kargv) {
	...
	//(1) create a new mm for current process
	//判断当前进程的mm是否已经被释放了
	if(current->mm != NULL){//要求当前进程管理器为null
    ...
    //读文件头 (3.1) read raw data content in file and resolve elfhdr
    ...
    //通过文件头找段表
    for (phnum = 0; phnum < elf->e_phnum; phnum ++) {//段数
        ...
        size_t off, size;
        uintptr_t start = ph->p_va, end, la = ROUNDDOWN(start, PGSIZE);//la为实际位置
        ...
        while (start < end) {
            ...
            if ((ret = load_icode_read(fd, page2kva(page) + off, size, offset)) != 0) {//调用sysfile_read
                goto bad_cleanup_mmap;
            }
            start += size, offset += size;//
        }
        //call pgdir_alloc_page to allocate pages for BSS, memset zero in these pages
    ...
    //setup argc, argv将参数压站
    uint32_t argv_size=0, i;
    for (i = 0; i < argc; i ++) {//计算参数长度
        argv_size += strnlen(kargv[i],EXEC_MAX_ARG_LEN + 1)+1;
    }

    uintptr_t stacktop = USTACKTOP - (argv_size/sizeof(long)+1)*sizeof(long);//通过参数长度得到站顶地址
