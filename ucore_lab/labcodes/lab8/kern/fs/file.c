...
//当前进程的打开文件数组current->fs_struct->filemap[]（fd_array->file数组）中的一个空闲元素（即还没用于一个打开的文件）
static int
fd_array_alloc(int fd, struct file **file_store) {
//    panic("debug");
    struct file *file = get_fd_array();//获取数组首地址
    if (fd == NO_FD) {
        for (fd = 0; fd < FILES_STRUCT_NENTRY; fd ++, file ++) {
...
file_open(char *path, uint32_t open_flags) {
    ...
    int ret;
    struct file *file;
    if ((ret = fd_array_alloc(NO_FD, &file)) != 0) {//分配一个file数据结构的变量，并获取文件表中的下标
        return ret;
    }
