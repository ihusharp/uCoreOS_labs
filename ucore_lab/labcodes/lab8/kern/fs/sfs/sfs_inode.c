...
/*  
 * sfs_io_nolock - Rd/Wr a file contentfrom offset position to offset+ length  disk blocks<-->buffer (in memory)
 * @sfs:      sfs file system
 * @sin:      sfs inode in memory
...
sfs_io_nolock(struct sfs_fs *sfs, struct sfs_inode *sin, void *buf, off_t offset, size_t *alenp, bool write) {
    ...
    *alenp = 0;
    ...
    int ret = 0;
    size_t size, alen = 0;//alen只指向中间块的偏移
    uint32_t ino;
  ...
  //LAB8:EXERCISE1 YOUR CODE HINT: call sfs_bmap_load_nolock, sfs_rbuf, sfs_rblock,etc. read different kind of blocks in file
	...
	*/
    ...
    if((blkoff=offset%SFS_BLKSIZE) != 0){
    	...
    	// 通过sfs_inode的第index个索引指向的block的索引值取出存到相应的指针指向的单元（ino_store）
        if((ret=sfs_bmap_load_nolock(sfs, sin, blkno, &ino)) != 0){
   	    	goto out;
   	    }
   	    //读取数据
   	    if((ret=sfs_buf_op(sfs, buf, size, ino, blkoff)) != 0){//此函数的offset要写为blkoff
   	    	goto out;
    	}
        //完成实际的读写功能
   	    alen += size;//实际指向位置
    ...
    while(nblks != 0){
    	...
    	//读取数据
    	if((ret=sfs_block_op(sfs, buf, ino, 1)) != 0){//从第一块偏移
    		goto out;
    	}
    	alen += size;
    	buf += size;
    ...
    if((size=endpos%SFS_BLKSIZE) != 0){
    	...
    	//读取数据
    	if((ret=sfs_buf_op(sfs, buf, size, ino, 0)) != 0){//从最后一块的buff的0开始
    		goto out;
    	}
    	alen += size;//alen表示的是实际指向位置
    }
 ...
 *              refers to.
 */
//node 是根目录“/”所对应的inode节点；
//path 是文件sfs_filetest1的绝对路径/sfs_filetest1，
//而 node_store 是经过查找获得的sfs_filetest1所对应的inode节点。
static int
