...
alloc_proc(void) {
     ...
     */
        ...
        list_init(&(proc->run_link));//list_init - initialize a new entry
        proc->time_slice = 0;//初始化时间片
        proc->lab6_run_pool.right = proc->lab6_run_pool.left = proc->lab6_run_pool.parent = NULL;
        proc->lab6_stride = 0;//设置步长
        proc->lab6_priority = 0;//设置优先级(步进反比
    }
    return proc;
}
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    */
    ...
    local_intr_save(intr_flag);
    {
        proc->pid = get_pid();
        hash_proc(proc);
        set_links(proc);  //这里调用了一个set link函数来设置进程之间的连接。
        //将原来的函数进行扩展，封装到set_links函数中执行
        /*list_add(&proc_list, &(proc->list_link));
        nr_process ++;*/
    }
    local_intr_restore(intr_flag);
...
do_exit(int error_code) {
    ...
    if (mm != NULL) {//表示是用户进程，则开始回收此用户进程所占用的用户态虚拟内存空间；
        lcr3(boot_cr3);//切换到内核态的页表上
        if (mm_count_dec(mm) == 0) {//表明这个mm没有再被其他进程共享，可以彻底释放进程所占的用户虚拟空间了
            exit_mmap(mm);
            put_pgdir(mm);
            mm_destroy(mm);//回收页目录、释放内存
        }
        current->mm = NULL;//回收完毕
    }
    ...
    {
        proc = current->parent;
        if (proc->wait_state == WT_CHILD) {//当前进程的父进程current->parent处于等待子进程状态：
            wakeup_proc(proc);//则唤醒父进程，来进行回收PCB
        }
        while (current->cptr != NULL) {//该进程还有子进程，即其子进程为孤儿进程，cptr指向最小的
        	//则需要把这些子进程的父进程指针设置为内核线程initproc
            proc = current->cptr;//此处相当于一个双向链表，取出最后的，放到init的最后面
            current->cptr = proc->optr;//proc->optr (proc's older sibling，进行迭代替换，直至等于NULL
    
            proc->yptr = NULL;
            if ((proc->optr = initproc->cptr) != NULL) {
                initproc->cptr->yptr = proc;//插入到子进程链表最后
            }
            proc->parent = initproc;//将孤儿进程parent设为init
            initproc->cptr = proc;
            if (proc->state == PROC_ZOMBIE) {
            	//某个子进程的执行状态是PROC_ZOMBIE，则需要唤醒initproc来完成对此子进程的最后回收工作。
...
load_icode(unsigned char *binary, size_t size) {
    ...
    //重新建立mm_map堆栈
    //明确用户栈的位置在用户虚空间的顶端，大小为256个页，即1MB
    ...
    memset(tf, 0, sizeof(struct trapframe));//先清空先前进程的中断帧，以重新设置
...
do_execve(const char *name, size_t len, unsigned char *binary, size_t size) {
    ...
    if (mm != NULL) {
        ...
        //引用计数减1之后，是否为0,为0表明没有进程再需要此进程所占用的内存空间
...
repeat:
    haskid = 0;
    if (pid != 0) {//如果pid!=0，表示只找一个进程id号为pid的退出状态的子进程
        proc = find_proc(pid);
        if (proc != NULL && proc->parent == current) {
            haskid = 1;
            if (proc->state == PROC_ZOMBIE) {
            	//如果此子进程的执行状态为PROC_ZOMBIE
            	//表明此子进程处于退出状态，需要当前进程（即子进程的父进程）完成对子进程的最终回收工作，
                goto found;
            }
        }
    }
    else {//如果pid==0，找任意一个处于退出状态的子进程
        proc = current->cptr;
    ...
    //如果子进程不处于僵尸状态（表明此子进程还没有退出），那么该进程会变成睡眠状态，
    //因为需要等待子进程退出，之后调用schedule函数挂起自己，选择其他进程执行。
  	//如果为僵尸状态，那么会清除该进程。
    if (haskid) {
