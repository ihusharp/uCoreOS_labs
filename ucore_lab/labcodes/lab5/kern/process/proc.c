...
alloc_proc(void) {
	 ...
	 */
        ...
        proc->wait_state = 0;
        proc->cptr = proc->yptr = proc->optr = NULL;//进程之间的关系
    }
    return proc;
}
...
proc_run(struct proc_struct *proc) {
    if (proc != current) {
        ...
        {
            current = proc;
            load_esp0(next->kstack + KSTACKSIZE);//低特权级读高特权级直接读就行
            lcr3(next->cr3);
...
good_mm:
    ...
    // PADDR - takes a kernel virtual address returns physical address.
    return 0;
